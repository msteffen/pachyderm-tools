...
// meld generates the command to view a collection of file diffs with meld
...
func vimdiff(tmpdir string, files []string, tmpfiles []*os.File) error {
	...
	buf := bytes.NewBuffer(nil)       // 'buf' will contain the vim script
	buf.WriteString("set diffopt=filler,vertical\n")
	buf.WriteString("edit " + path.Join(GitRoot, files[0]) + "\n")
	buf.WriteString("diffsplit " + tmpfiles[0].Name() + "\n")
	for i := 1; i < len(files); i++ {
		buf.WriteString("tabe " + path.Join(GitRoot, files[i]) + "\n")
		buf.WriteString("diffsplit " + tmpfiles[i].Name() + "\n")
	}
	buf.WriteString("tabfirst\n")
	_, err = vimscript.Write(buf.Bytes())
	if err != nil {
		return fmt.Errorf("could not write out vimscript for diff: %s", err)
	}
	err = vimscript.Close()
	if err != nil {
		return fmt.Errorf("could not close vimscript for diff: %s", err)
	}
